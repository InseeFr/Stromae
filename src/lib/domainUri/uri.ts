...
// If survey doesn't exist, the url returns `/423`
// If survey exists, the url returns `/questionnaire/:survey/423/temporairement-indisponible`
// Depending on the error message, the last parameter can be replaced by `post-collecte` or `pre-collecte`
export function uri423(survey?: string, message?: string) {
	const surveyUrl = survey ? `questionnaire/${survey}/423` : '423';
	let status = '/temporairement-indisponible';
	// 423 error can be reached either before or after the collection period
