                ...
                <p:input name="config">
                        <parametres xsl:version="2.0">
                                ...
                                <xsl:variable name="uni">
                                        <xsl:value-of select="doc('input:parameters')/parameters/text()"/>
                                </xsl:variable>
                                
                                <xsl:variable name="uni">
                                        <xsl:value-of select="doc('input:parameters')/parameters/surveyUnit/text()"/>
                                </xsl:variable>
                                <xsl:variable name="enq">
                                        <xsl:value-of select="doc('input:parameters')/parameters/survey/text()"/>
                                </xsl:variable>
                                <xsl:variable name="mod">
                                        <xsl:value-of select="doc('input:parameters')/parameters/modele/text()"/>
                                </xsl:variable>
                                <xh:html>
                                        <xh:head>
                                                <xf:model id="fr-model" xxf:optimize-get-all="false" xxf:no-updates="true" xxf:local-submission-forward="false">
                                                        ...
                                                        <xf:submission id="get-pdf" replace="all" method="post" ref="/" resource="{p:property('server-exist-orbeon')}/rest/db/WS/store-pdf.xquery?unite={$uni}&amp;modele={$mod}&amp;enquete={$enq}">
                                                        </xf:submission>
