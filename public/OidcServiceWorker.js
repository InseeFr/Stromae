...
const handleFetch = async (event) => {
	...
	if (
		...
		const newRequest = new Request(originalRequest, {
			...
			mode: currentDatabaseForRequestAccessToken.oidcConfiguration
	...
	if (numberDatabase > 0) {
		const maPromesse = new Promise((resolve, reject) => {
			...
			const response = clonedRequest.text().then((actualBody) => {
				if (
					actualBody.includes(REFRESH_TOKEN) ||
					actualBody.includes(ACCESS_TOKEN)
				) {
					...
					for (let i = 0; i < numberDatabase; i++) {
						...
						if (currentDb && currentDb.tokens != null) {
							...
							const keyAccessToken =
								ACCESS_TOKEN + '_' + currentDb.configurationName;
					...
					if (currentDatabase && currentDatabase.codeVerifier != null) {
						const keyCodeVerifier =
							CODE_VERIFIER + '_' + currentDatabase.configurationName;
...
addEventListener('message', (event) => {
	...
	switch (data.type) {
		...
		case 'getCodeVerifier': {
			port.postMessage({
				configurationName,
				codeVerifier: CODE_VERIFIER + '_' + configurationName,
			});
			return;
		}
